...
while(1):
    if i >= 10 and i % 2 == 0:
        ...
        model = tf.keras.models.load_model(f'models/{NAME}')
        i = 0
    # GUARANTEE THAT HE WILL BET ON THE SECOND, BECAUSE HERE HE ALREADY GETS THE DICE OF ONE IN FRONT.
    if datetime.datetime.now().second < 30 and i % 2 == 0:
        time_taker = time.time()
        # THEREFORE, HE NEEDS TIME TO PREPARE THE FORECAST BEFORE IT REACHES 59 SECONDS FOR HIM
        pred_ready = preprocess_prediciton(iq)
        # MAKE THE BET, AND NO HE WILL TRY TO PREDICT THE VALUE OF THE THIRD FRONT
